...
The client is a command line tool that can connect to the open socket and captures the stream of heap operations from the socket connection. The client is a JVM application written in Kotlin. The client can capture, visualise, and load/store the memory trace generated by the server.
